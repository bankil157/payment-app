            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Select 
                  ...
                  disabled={!formData.paymentId}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={!formData.paymentId ? "Select payment first" : "Select invoice"} />
                  </SelectTrigger>
                  <SelectContent>
                ...
                {!formData.paymentId && (
                  <p className="text-xs text-muted-foreground mt-1">
                    Please select a payment first to see eligible invoices
                  </p>
                )}
                {selectedPayment && getAvailableInvoices().length === 0 && formData.paymentId && (
                  <p className="text-xs text-red-600 mt-1">
                    No eligible invoices found. Payment from {selectedPayment.receivedFrom} can only be allocated to invoices containing this {selectedPayment.receivedFromType}.
                  </p>
                )}
                {selectedInvoice && (
                  <p className="text-xs text-muted-foreground mt-1">
...
<to_replace>                <Select value={formData.paymentId} onValueChange={(value) => setFormData(prev => ({ ...prev, paymentId: value }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select payment" />
                  </SelectTrigger></to_replace>
<new_content>                <Select value={formData.paymentId} onValueChange={(value) => setFormData(prev => ({ ...prev, paymentId: value, invoiceId: '' }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select payment" />
                  </SelectTrigger>
